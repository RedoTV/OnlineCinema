...
// Факт «контент реально досмотрели» (один настоящий просмотр).
...
//
// Смотрят и гости без аккаунта: тогда UserId = null, а зритель различается
// по ViewerKey — устойчивому идентификатору браузера (localStorage).
public class UserWatch
{
    public int Id { get; set; }

    // null = анонимный просмотр гостя.
    public int? UserId { get; set; }

    // Устойчивый id зрителя без аккаунта (guid из localStorage).
    // У залогиненных — тоже заполняется (тот же localStorage-ключ).
